...
assert(result.compression.ratio > 1, `ratio > 1 (${result.compression.ratio.toFixed(2)})`);
assert(
...
assert(expanded.messages_expanded > 0, `messages expanded (${expanded.messages_expanded})`);
assert(expanded.missing_ids.length === 0, `no missing IDs`);
assert(
  messages.map((m) => m.content).join('|') === expanded.messages.map((m) => m.content).join('|'),
...
assert(typeof count === 'number' && count > 0, `returns positive number (${count})`);
...
assert(typeof createEscalatingSummarizer === 'function', 'createEscalatingSummarizer exported');

console.log('\n11. forceConverge (best-effort truncation)');
const fcResult = compress(messages, { tokenBudget: 200, forceConverge: true });
assert(
  fcResult.tokenCount <= compress(messages, { tokenBudget: 200 }).tokenCount,
...
assert(fuzzyResult.messages.length === messages.length, `message count preserved`);
...
if (compMsg) {
  ...
  assert(Array.isArray(orig.ids) && orig.ids.length > 0, `_cce_original.ids is non-empty array`);
...
assert(singleResult.compression.messages_preserved === 1, `single message counted as preserved`);
...
console.log('\n17. Async path (mock summarizer)');
{
  ...
  assert(asyncResult.messages.length === messages.length, `message count preserved`);
  assert(asyncResult.compression.messages_compressed > 0, `messages compressed via summarizer`);
  assert(Object.keys(asyncResult.verbatim).length > 0, `verbatim store populated`);
  // Round-trip the async result
  const asyncExpanded = uncompress(asyncResult.messages, (id) => asyncResult.verbatim[id] ?? null);
...
console.log('\n19. System role auto-preserved');
{
  ...
  assert(!sysMsg.metadata?._cce_original, `system message not compressed (no _cce_original)`);
  assert(sysMsg.content === withSystem[0].content, `system message content untouched`);
}
...
console.log('\n20. Messages with tool_calls pass through');
{
  ...
  assert(toolMsg.tool_calls[0].function.name === 'get_weather', `tool_calls content intact`);
}
...
console.log('\n21. Re-compression (compress already-compressed output)');
{
  ...
  assert(recovered.messages_expanded > 0, `recursive uncompress expanded messages`);
